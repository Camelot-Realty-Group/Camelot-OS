...
CONSOLIDATION NOTE
------------------
This file may be added independently on the sibling `feature/costbeat-bot`
branch as well (CostBeat needs the same Spire budget pull for its own
pipeline). That is expected duplication — this module already lives in the
shared `utils/` package on both branches, so once both PRs merge the two
copies need to be reconciled (they should be near-identical; keep whichever
lands second and delete the other, diffing for any drift first).
...
class SpireError(Exception):
    """Base class for all Spire client errors."""
...
class SpireAPIError(SpireError):
    """Raised when Spire responds with an error Spire client code can't recover from."""
...
class SpireClient:
    ...
    def __init__(self) -> None:
        ...
        client_secret = os.getenv("SPIRE_CLIENT_SECRET", "")
        ...
        self.base_url = os.getenv("SPIRE_BASE_URL", DEFAULT_BASE_URL).rstrip("/")
